...
SAMPLE_PAGE_DATA_IN_PAYMENT_LINE = """\
Forma de Pagamento: Remessa Bancária Nº do Cheque: Nº do Comprovante: 66.848/66.856
Nº Processo Credor Imposto Nota Empenho Nº Lançamento Data Valor
2026PG00161 BANCO DO BRASIL S/A 2026NE00129 01681/2026 06/02/2026 3.262,00
2026PG00161 BANCO DO BRASIL S/A 2026NE00129 01683/2026 06/02/2026 466,00
"""
...
@patch('toolbox.siscac_payments.PdfReader')
def test_parse_siscac_report_extracts_date_from_payment_line(mock_reader_class):
    mock_reader_class.return_value = _make_fake_reader([SAMPLE_PAGE_DATA_IN_PAYMENT_LINE])

    result = parse_siscac_report(b'fake pdf bytes')

    assert len(result) == 1
    row = result[0]
    assert row['siscac_pg'] == '2026PG00161'
    assert row['data_contabilizado'] == '06/02/2026'
    assert row['comprovante'] == '66848'
