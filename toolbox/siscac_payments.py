...
def parse_siscac_report(pdf_source) -> list[dict]:
    ...
    """
    # Layout novo: PG  Credor  [NE]  Nº Lançamento  Data  Valor
    # Ex.: 2026PG00161 BANCO ... 2026NE00129 01681/2026 06/02/2026 3.262,00
    pattern_payment_with_date = re.compile(
        r'^(20\d{2}PG\d{5})\s+(.*?)\s+(?:20\d{2}NE\d{5}\s+)?\d{5}/\d{4}\s+([\d]{2}/[\d]{2}/[\d]{4})\s+([\d.,]+)$'
    )
    # Layout legado: PG  Credor  NE  ...  Valor
    pattern_payment_legacy = re.compile(
    ...
    for page in reader.pages:
        ...
        for line in text.splitlines():
            ...
            m_pay = pattern_payment_with_date.match(line)
            has_date_in_line = bool(m_pay)
            if not m_pay:
                m_pay = pattern_payment_legacy.match(line)

            if m_pay:
                pg = m_pay.group(1)
                credor = m_pay.group(2).strip()
                if has_date_in_line:
                    payment_date = m_pay.group(3)
                ...
                if pg in payments:
                    ...
                    if not payments[pg].get('data_contabilizado') and payment_date:
                        payments[pg]['data_contabilizado'] = payment_date
                else:
                    payments[pg] = {
                        'siscac_pg': pg,
                        'credor': credor,
                        'valor_total': valor_decimal,
                        'data_contabilizado': payment_date or current_data,
